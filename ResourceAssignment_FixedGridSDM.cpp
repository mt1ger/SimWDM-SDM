...
void ResourceAssignment::check_availability_link (vector<int> * circuitRoute, vector<int> * WLsforAllocation, unsigned int * WLCounter, int WL) {
			...
			if (WLCounter == OccupiedWL) {
				for (int j = 1; j < CircuitRoute.size (); j++) {
					for (int k = 0; k < WLsforAllocation.size (); k++) {
						network->Wavelengths[CircuitRoute[j - 1]][CircuitRoute[j]][0][WLsforAllocation[k]] = true;
					}
				}
				break;
			}
...
void ResourceAssignment::handle_releases (CircuitRelease * circuitRelease) {
	...
	for (int i = 1; i < circuitRelease->CircuitRoute.size (); i++) {
		for (int j = 0; j < circuitRelease->WLAllocList.size (); j++) {
			network->Wavelengths[circuitRelease->CircuitRoute[i - 1]][circuitRelease->CircuitRoute[i]][0][circuitRelease->WLAllocList[j]] = false;	
		}
	}
	...
	for (int i = 0; i < circuitRelease->WLAllocList.size (); i++) {
		cout << circuitRelease->WLAllocList[i] << ' ';
	}
	cout << endl;
	cout << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << endl << endl;
	#endif
